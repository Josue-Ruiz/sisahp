...
@section('content')
    ...
    @include('components.alerts.all')
            <div class="x_panel">
                  <div class="x_title">
                    <h2>Direcciones por localidad<small></small></h2>
                    <ul class="nav navbar-right panel_toolbox">
                   <li>
                        <a href="{{ route('calles-localidad.create',['municipio'=>$municipalitie,'localidad'=>$location] ) }}"><button class="btn btn-sm btn-success" type="button"><i class="fa fa-plus"></i> Agregar</button></a>
                   </li>
                    </ul>
                      ...
                      <tbody>
                        ...
                        @foreach($registers as $key=>$item)
                            <tr>
                                ...
                                <a href="{{ route('calles-localidad.edit',$item->id) }}"><button class="btn btn-sm btn-warning" type="button"><i class="fa fa-edit"></i> Editar</button></a>
                                ...
                                @include('components.alerts.confirm',['ruta'=>route('calles-localidad.destroy',$item->id),'id'=>$item->id])
                        @endforeach
                ...
                @endif
                <a href="{{  route('localidades.index',['municipio'=>$municipalitie]) }}" class="btn btn-dark"> <i class="fa fa-arrow-left"></i> Regresar</a>
                  </div>
                </div>
              </div>
